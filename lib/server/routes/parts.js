...
router.get('/parts', function(req, res){ // parts/parts baby
  console.log(spaceshipParts.spaceshipParts);
  res.send(spaceshipParts.spaceshipParts);
});
...
router.get('/rocketCount', function(req, res){ // count your rockets or rocket your count?
  console.log('parts/rocketCount route has been reached');
  ...
  console.log('howMany.count is: ',howMany.count);
  res.send(howMany.count);  // numberOfSpaceships is a number, how does express like that? howMany ways to say this...
});
