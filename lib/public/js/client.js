$(document).ready(function(){
  ...
  function getParts(){
    $.ajax({
      ...
      success: function(response){
        console.log('getParts response received.  it is: ',response);
        displayParts(response);
      },
      error: function(error){
  ...
  function getRocketPossibleCount(){
    console.log('getRocketPossibleCount has been called...');
    $.ajax({
      type: 'GET',
      url: 'parts/rocketCount',
      success: function(data){
        console.log('the getRocketPossibleCount response is: ',data);
